...
## Discord任意シェルとゲームプロセスの権限分離 (2026-07)
...
任意シェルは運用要件であるため撤廃できない。一方、MinecraftプラグインやModがBotと同一UID・環境で動作すると、Discord／Notionトークンやホスト管理権限へ到達できる。操作主体の厳格な限定とゲームプロセスの権限分離を併用する構成を採用する。

### デプロイ時の停止・復旧

ゲームプロセスはDiscordログ転送のためBotの子プロセスとして維持する。デプロイ前に稼働中サーバーをdesired stateへ保存し、RCONで安全に停止する。Bot起動後はdesired stateから順次起動し、RCONまたはゲームポートのready確認後に復旧完了とする。
